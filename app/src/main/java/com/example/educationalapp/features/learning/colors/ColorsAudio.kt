...
class ColorsAudioManager(private val context: Context) {
    ...
    /**
     * @param onComplete Callback apelat când sunetul s-a terminat de redat.
     */
    fun playVoice(resName: String, interrupt: Boolean = true, onComplete: (() -> Unit)? = null) {
        if (interrupt) stopVoice()

        // Verificăm dacă playerul de muzică este valid înainte de ducking
        try {
        ...
        duckJob = scope.launch {
            ...
            for (i in 1..steps) {
                try {
                    ...
                    // Doar dacă playerul există și este încă activ (nu eliberat)
                    if (player != null && player.isPlaying) {
                        val vol = 0.05f + (0.25f * (i.toFloat() / steps))
                        player.setVolume(vol, vol)
                    }
                } catch (e: IllegalStateException) {
                    // Playerul a fost probabil eliberat sau oprit între timp
                    break
                } catch (e: Exception) {
                    break
                }
